...
def main():
    ...
    count = 1
    predictions = None
    objective = kt.Objective("val_custom_weighted_accuracy", direction="max")
    for target_id in training_data:
        ...
        tuner = kt.Hyperband(
            ...
            directory="tuner_dir",
            project_name="qrt_data_challenge_2021_" + str(target_id),
        )
        ...
        val_acc_per_epoch = history.history["val_custom_weighted_accuracy"]
        ...
        if predictions:
            ...
            predictions = (
                ...
                np.append(preds, np.sign(hypermodel.predict(x_test) - 0.5)),
            )
        else:
            preds = np.sign(hypermodel.predict(x_test) - 0.5)
            (n, _) = preds.shape
